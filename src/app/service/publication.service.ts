...
export class PublicationService {
  ...
  public getPublications(): Observable<Array<Publication>> {
    return this.http.get<Array<Publication>>('Http://localhost:8087/api/v1/auth/retrieveAllPofForum');
  }

  getPublicationById(idpub: number): Observable<Publication> {
    return this.http.get<Publication>(`Http://localhost:8087/api/v1/auth/retrieveByIdPub/${idpub}`);
  }


  searchBySubject(query: string): Observable<Array<Publication>> {
    return this.http.get<Array<Publication>>('Http://localhost:8087/api/v1/auth/retrieveBySubject?subject=' + query);
  }

  searchByContent(query: string): Observable<Array<Publication>> {
    return this.http.get<Array<Publication>>('Http://localhost:8087/api/v1/auth/retrieveByContent?content=' + query);
  }
  ...
  addPublication(publication: {
    ...
    return this.http.post<Publication>(`Http://localhost:8087/api/v1/auth/${idUSER}/1/addPub`, publication);
  }
  ...
  deletePublication(id: number): Observable<any> {
    return this.http.delete<any>(`Http://localhost:8087/api/v1/auth/deletePub/${id}`);
  }

  updatePublication(publication: Publication): Observable<Publication> {
    return this.http.put<Publication>('Http://localhost:8087/api/v1/auth/updatePub', publication);
  }

  fetchCommentsByPublicationId(idpub: number): Observable<Array<Commentary>> {
    return this.http.get<Array<Commentary>>(`Http://localhost:8087/api/v1/auth/retrieveALLC/${idpub}`);
  }
  incrementViewsForPublication(id: number): Observable<any> {
    return this.http.put(`Http://localhost:8087/api/v1/auth/publications/${id}/increment-views`, {});
  }
  searchByTags(tag: string): Observable<Array<Publication>> {
    return this.http.get<Array<Publication>> ('Http://localhost:8087/api/v1/auth/retrieveByTagsP?tags=' + tag);
  }
  fetchPubByIdUser(idUser: number): Observable<Publication[]> {
    return this.http.get<Publication[]>(`Http://localhost:8087/api/v1/auth/retrieveByUser/${idUser}`);
  }
  uploadImage(imageFile: File, idpub: number): Observable<any> {
    const formData: FormData = new FormData();
    formData.append('imageFile', imageFile, imageFile.name);
    return this.http.post(`http://localhost:8087/api/v1/auth/upload/${idpub}`, formData);
  }
  }
